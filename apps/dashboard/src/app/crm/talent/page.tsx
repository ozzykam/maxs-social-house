...
export default function TalentBookingsPage() {
  ...
  useEffect(() => {
    listInquiriesByType("talent").then((data) => {
      setInquiries(data);
      setLoading(false);
    });
  }, []);
    ...
    <PermissionGate anyOf={["crm:read"]}>
      <div className="max-w-5xl mx-auto px-6 py-8">
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-semibold text-gray-900 mt-1">Talent Bookings</h1>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white border border-gray-200 rounded-xl px-8 py-12 text-center">
            <p className="text-sm text-gray-400">No talent inquiries yet.</p>
          </div>
        ) : (
            ...
            {inquiries.map((inq) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-gray-700 truncate">{inq.artistName ?? "—"}</p>
                  <p className="text-xs text-gray-400 mt-0.5 truncate">{inq.genre ?? "No genre specified"}</p>
                </div>
